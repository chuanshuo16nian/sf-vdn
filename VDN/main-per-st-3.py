...
class VDN(object):
    ...
    def experience_replay(self, state, action, reward, next_state, terminal):
        ...
        #     # #############################################################
        if self.progress == 'Exploring':
            p = pow(abs(reward) + self.eps, self.alpha)
            self.replay_memory.add(p, [state, action, reward, next_state, terminal])
        elif self.progress == 'Training':
            Q = self.output_target.eval(feed_dict={self.input_target_1:[next_state[0]],
                                                   self.input_target_2:[next_state[1]]})
            if terminal == True:
            ...
            TD_error = self.TD_error.eval(feed_dict={self.action_target:[act],
                                                     ...
                                                     self.input_1:[state[0]],
                                                     self.input_2:[state[1]]})[0]
            p = pow((abs(TD_error) + self.eps), self.alpha)
            self.replay_memory.add(p, [state, action, reward, next_state, terminal])
    ...
    def prioritized_minibatch(self):
        ...
        w_batch = []
        batch_index = []
        segment = self.replay_memory.total() / self.Num_batch
        min_prob = np.min(self.replay_memory.tree[-self.replay_memory.capacity:]) / self.replay_memory.total()
        for i in range(self.Num_batch):
            a = segment * i
            b = segment * (i + 1)
            s = random.uniform(a, b)
            (idx, p, data) = self.replay_memory.get(s)
            prob = p / self.replay_memory.total()
            is_w = pow(prob/min_prob, -self.beta)
            w_batch.append(is_w)
    ...
    def train(self, minibatch, w_batch, batch_index):
        ...
        print(action_1_batch)
        print(action_2_batch)
        # get y_prediction
        y_batch = []
        ...
        # Update p in the sum_tree
        for i in range(len(minibatch)):
            idx = batch_index[i]
            p = pow(abs(TD_error_batch[i]) + self.eps, self.alpha)
            self.replay_memory.update(idx, p)
